...
// generated on 2026-07-02T08:35:18.453720164Z
...
class CustomerSummary
{
    ...
    (
        ...
        public readonly bool|null $affiliateContactAllowed,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool|null $affiliateContactAllowed */
        $affiliateContactAllowed = $decodedJson['affiliateContactAllowed'];

        return new CustomerSummary
        (
            $customerId, $companyName, $firstName, $lastName, $city, $canLoginAsAffiliate, $affiliateContactAllowed
        );
    }
}
...
class CreateDnsPlanInput
{
    /**
     * @param $domainName The domain name. For internationalized domain names, supported are both 'punycode' encoded domain names with the ACE prefix, or Unicode domain names (but the two formats should never be mixed). 
     * @param (array{string,string})[] $dnsTemplateVars
     */
    public function __construct
    (
        public readonly string $domainName,
        public readonly int $dnsTemplateId,
        public readonly array $dnsTemplateVars,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var int $dnsTemplateId */
        $dnsTemplateId = $decodedJson['dnsTemplateId'];

        $var0 = array();
        foreach ($decodedJson['dnsTemplateVars'] as $element0) {
            $var1 = array();
            $var1[] = $element0[0];
            $var1[] = $element0[1];
            $var0[] = $var1;
        }
        /** @var (array{string,string})[] $dnsTemplateVars */
        $dnsTemplateVars = $var0;

        /** @var string $confirmationText */
        $confirmationText = $decodedJson['confirmationText'];

        return new CreateDnsPlanInput
        (
            $domainName, $dnsTemplateId, $dnsTemplateVars, $confirmationText
        );
    }
}
...
{
    /**
     * @param \ShockMedia\Generated\Domainregistration\UnstartedDomainRegistration\Info[] $domains The unstarted domain registration info, for each specified domain.
     */
    public function __construct
    (
...
{
    /**
     * @param $role The role of this domain contact.
     * @param $organization Optionally the name of the organization of the contact. Note: registries typically treat organizations and individuals very differently, so this should not be set just because some organization name may be available. 
     * @param $name The name of the contact.
     * @param $address The street and house number.
     * @param $postalCode The postal code in a format that is valid for the specified 'country' value.
     * @param $city The city.
     * @param $state Optionally a state or province.
     * @param $country The valid ISO 3166-1 alpha2 country code, which is always two capital case letters.
     * @param $email A valid email address. Mails may be send to this address by us and/or the registry.
     * @param $telVoice A telephone number in E164a format, eg: +31.384530759 
     * @param $telFax Optionally a fax number in E164a format, eg: +31.384530759 
...
/**
 * Information that can be used to create a new contact.
 * See, e.g., 'CreateDomainRequestContactInput' for more information.
 */
...
class CreateDnsRecordInput
{
     ...
     * @param $ttl Optionally the TTL of the record.
     * @param $prio Optionally the priority of the record (supported by only a few record types).
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        return new CreateDnsRecordInput
        (
...
{
    /**
     * @param $handle The (generated) identifier of the domain contact.
     * @param $organization Optionally the name of the organization of the contact. Note: registries typically treat organizations and individuals very differently. 
     * @param $name The name of the domain contact.
     * @param $address The address of the domain contact.
     * @param $postalCode The postal code of the domain contact, in a format that is valid for the specified 'country' value.
     * @param $city The city of the domain contact.
     * @param $state The state or provice of the contact (optional).
     * @param $country The country of the contact. A ISO 3166-1 alpha2 country code, which is always two capital case letters. 
     * @param $email The email adres of the domain contact.
     * @param $telVoice The telephone number of the domain contact, in E164a format.
     * @param $telFax Optionally a fax number of the domain contact, in E164a format if set.
     * @param \ShockMedia\Generated\Domainregistration\DomainContact\ExtraProperty[] $extraProperties Additional properties that are only used by specific registries.
     * @param \ShockMedia\Generated\Domainregistration\DomainContact\Validation[] $validations The validations that have been comleted for the domain contact.
     */
    public function __construct
    (
...
{
    /**
     * @param $authCode The auth code to use for the incoming transfer. Some registries do not support an auth code, others always require one, and yet others support both. 
     * @param $domainName The domain name. For internationalized domain names, supported are both 'punycode' encoded domain names with the ACE prefix, or Unicode domain names (but the two formats should never be mixed). 
     * @param $reuseRegistrantHandle Optionally the handle of a previously created contact that is to be reused for this domain name as registrant. 
     * @param $reuseAdminHandle Optionally the handle of a previously created contact that is to be reused for this domain name as admin contact. 
     * @param $planType What kind of plan is to be acquired: just a domain registration plan (`domreg`), or also a hosting plan (`hosting`). 
     * @param $hostingPlanId If `hosting` is specified as plan type, the hosting plan ID must be specified here.
     * @param $nameserversType Whether to use our nameservers (`smbv`), or nameservers specified by the customer (`my`).
     * @param $configDomain The domain configuration. `regpage` to show the default Shock Media registration page, `server` to point a domain to a particular server, `dnstpl` to create a DNS zone and apply a DNS template. 
     * @param $serverName If `server` was specified for the domain configuration, a valid server name of a server owned by the customer must be specified. 
     * @param $dnsTemplateId If `dnstpl` was specified for the domain configuration, a valid DNS template ID must be specified. 
     * @param $ipv4 If a DNS template was specified, the IPv4 adres is a mandatory template variable. 
     * @param $ipv6 If a DNS template was specified, the IPv6 adres is an optional template variable. 
     * @param \ShockMedia\Generated\Domainregistration\CreateDomainRequestInput\NameServer[]|null $nameServers If `my` was specified as name servers type, the nameservers that are to be used must be provided here. 
     * @param \ShockMedia\Generated\Domainregistration\CreateDomainRequestContactInput[]|null $contacts
     * @param \ShockMedia\Generated\Domainregistration\CreateDnsRecordInput[]|null $dnsRecords
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        if ($decodedJson['dnsRecords'] === NULL) {
            ...
            foreach ($decodedJson['dnsRecords'] as $element0) {
                $var0[] = \ShockMedia\Generated\Domainregistration\CreateDnsRecordInput::fromDecodedJson($element0);
            }
        }
        /** @var \ShockMedia\Generated\Domainregistration\CreateDnsRecordInput[]|null $dnsRecords */
        $dnsRecords = $var0;
        ...
        (
            $authCode, $domainName, $reuseRegistrantHandle, $reuseAdminHandle, $planType, $hostingPlanId, $nameserversType, $configDomain, $serverName, $dnsTemplateId, $ipv4, $ipv6, $nameServers, $contacts, $dnsRecords, $quotes
        );
    }
}
...
{
    /**
     * @param $registry The (internal) name of the registry.
...
namespace ShockMedia\Generated\Domainregistration\UnstartedDomainRegistration;


/**
 * The unstarted domain registration info for a single domain.
 */
class Info
{
    /**
     * @param $domain The name of the domain.
     * @param $gatheringInProgress Whether information about the request is still be obtained. If true, then one or more of the following fields may no longer be null upon re-executing the request: status, basicInfo, contactConfiguration, planConfiguration. If false, then, all else being equal, those fields will remain null. 
     * @param $status The status of the domain, if available.
     * @param $basicInfo Basic pricing and transfer information, if currently available.
     * @param $contactConfiguration Information regarding possible contacts, if currently available.
     * @param $planConfiguration Information regarding plan configuration, if currently available.
     */
    public function __construct
    (
        public readonly string $domain,
        public readonly bool $gatheringInProgress,
        public readonly \ShockMedia\Generated\Domainregistration\DomainCheckStatus|null $status,
        public readonly \ShockMedia\Generated\Domainregistration\Info\BasicInfo|null $basicInfo,
        public readonly \ShockMedia\Generated\Domainregistration\Info\ContactConfiguration|null $contactConfiguration,
        public readonly \ShockMedia\Generated\Domainregistration\Info\PlanConfiguration|null $planConfiguration,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool $gatheringInProgress */
        $gatheringInProgress = $decodedJson['gatheringInProgress'];
        ...
        if ($decodedJson['basicInfo'] === NULL) {
            $basicInfo = NULL;
        } else {
            $basicInfo = \ShockMedia\Generated\Domainregistration\Info\BasicInfo::fromDecodedJson($decodedJson['basicInfo']);
        }

        if ($decodedJson['contactConfiguration'] === NULL) {
            $contactConfiguration = NULL;
        } else {
            $contactConfiguration = \ShockMedia\Generated\Domainregistration\Info\ContactConfiguration::fromDecodedJson($decodedJson['contactConfiguration']);
        }

        if ($decodedJson['planConfiguration'] === NULL) {
            $planConfiguration = NULL;
        } else {
            $planConfiguration = \ShockMedia\Generated\Domainregistration\Info\PlanConfiguration::fromDecodedJson($decodedJson['planConfiguration']);
        }

        return new Info
        (
            $domain, $gatheringInProgress, $status, $basicInfo, $contactConfiguration, $planConfiguration
        );
    }
}

namespace ShockMedia\Generated\Domainregistration\Info;


/**
 * Basic pricing and transfer information.
 */
class BasicInfo
{
    /**
     * @param $monthly The monthly price of the domain registration.
     * @param $initialPeriod The initial contract period.
     * @param $renewalPeriod The renewal contract period.
     * @param $discountedMonthly The discounted monthly price of the domain registration, if applicable.
     * @param $otherMax The maximum one-time surcharge that might be applied.
     * @param $discountedOtherMax The discounted maximum one-time surcharge that might be applied, if applicable.
     * @param $transferInfo Additional information if the domain request is for an incoming transfer.
     */
    public function __construct
    (
        ...
        public readonly float|null $discountedOtherMax,
        public readonly \ShockMedia\Generated\Domainregistration\BasicInfo\TransferInfo|null $transferInfo,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        $initialPeriod = $decodedJson['initialPeriod'];

        /** @var int $renewalPeriod */
        $renewalPeriod = $decodedJson['renewalPeriod'];
        ...
        /** @var float|null $discountedOtherMax */
        $discountedOtherMax = $decodedJson['discountedOtherMax'];

        if ($decodedJson['transferInfo'] === NULL) {
            $transferInfo = NULL;
        } else {
            $transferInfo = \ShockMedia\Generated\Domainregistration\BasicInfo\TransferInfo::fromDecodedJson($decodedJson['transferInfo']);
        }

        return new BasicInfo
        (
            $monthly, $initialPeriod, $renewalPeriod, $discountedMonthly, $otherMax, $discountedOtherMax, $transferInfo
        );
    }
}

namespace ShockMedia\Generated\Domainregistration\BasicInfo;


/**
 * Additional information related to the specified incoming transfer.
 */
class TransferInfo
{
    /**
     * @param $authCodeSupported Whether the registry supports an auth code.
     * @param $authCodeRequired Whether the registry requires an auth code.
     * @param $requiresFOA Whether a 'Form of Authorization' is required.
     */
    public function __construct
    (
...
/**
 * Information regarding possible contacts.
 */
class ContactConfiguration
{
    /**
     * @param $suffix The registry suffix (TLD).
     * @param $registry The (internal) name of the registry.
     * @param \ShockMedia\Generated\Domainregistration\ContactConfiguration\ContactProperty[] $contactProperties Additional properties that can be set, targeting a specific registry.
     * @param $minAdmin The minimum number of 'admin' contacts.
     * @param $maxAdmin The maximum number of 'admin' contacts.
     * @param \ShockMedia\Generated\Domainregistration\DomainContact[] $availableRegistrantContacts Currently available contacts for the role 'registrant'.
     * @param \ShockMedia\Generated\Domainregistration\DomainContact[] $availableAdminContacts Currently available contacts for the role 'admin'.
     * @param $contactInputSources Information that can be used to create new domain contacts.
     */
    public function __construct
    (
        ...
        public readonly \ShockMedia\Generated\Domainregistration\ContactConfiguration\ContactConfigurationContactInputSources $contactInputSources,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        foreach ($decodedJson['contactProperties'] as $element0) {
            $var0[] = \ShockMedia\Generated\Domainregistration\ContactConfiguration\ContactProperty::fromDecodedJson($element0);
        }
        /** @var \ShockMedia\Generated\Domainregistration\ContactConfiguration\ContactProperty[] $contactProperties */
        ...
        $contactInputSources = \ShockMedia\Generated\Domainregistration\ContactConfiguration\ContactConfigurationContactInputSources::fromDecodedJson($decodedJson['contactInputSources']);
...
namespace ShockMedia\Generated\Domainregistration\ContactConfiguration;


/**
 * A contact property that is specific to a certain registry.
 */
class ContactProperty
{
    /**
     * @param $name The (internal) name of the property; sometimes referred to by other properties.
     * @param $label A more human readable label of the property name (always in English).
     * @param $description A more verbose description of the property (always in English).
     * @param $type The type of the possible value of the property.
     * @param $mandatory If the property is always mandatory. If set to false, the property may still be mandatory under specific circumstances. 
     * @param $values If the property is restricted to specific values, those values will be in this object, mapped to a description. 
     */
    public function __construct
    (
        public readonly string $name,
...
/**
 * Information that can be used to create new domain contacts.
 */
class ContactConfigurationContactInputSources
{
    /**
     * @param $contractor Information that can be used to create a new contact. See, e.g., 'CreateDomainRequestContactInput' for more information. 
     * @param \ShockMedia\Generated\Domainregistration\CreateDomainContactInputSource[] $others Information based on other customer account data
     */
    public function __construct
    (
        public readonly \ShockMedia\Generated\Domainregistration\CreateDomainContactInputSource $contractor,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        $contractor = \ShockMedia\Generated\Domainregistration\CreateDomainContactInputSource::fromDecodedJson($decodedJson['contractor']);

        $var0 = array();
        foreach ($decodedJson['others'] as $element0) {
            $var0[] = \ShockMedia\Generated\Domainregistration\CreateDomainContactInputSource::fromDecodedJson($element0);
        }
        /** @var \ShockMedia\Generated\Domainregistration\CreateDomainContactInputSource[] $others */
        $others = $var0;

        return new ContactConfigurationContactInputSources
        (
            $contractor, $others
        );
    }
...
/**
 * Information regarding plan configuration.
 */
class PlanConfiguration
{
    /**
     * @param string[] $servers
     * @param \ShockMedia\Generated\Domainregistration\PlanConfiguration\DnsTemplateRef[] $dnsTemplates Available DNS templates.
     * @param \ShockMedia\Generated\Domainregistration\PlanConfiguration\HostingPlan[] $hostingPlans Available hosting plans.
     * @param \ShockMedia\Generated\Domainregistration\PlanConfiguration\OfferingSpecification[] $specs The specifications of our offerings related to the domain.
     * @param $nameservers The nameserver requirements.
     */
    public function __construct
    (
        ...
        public readonly \ShockMedia\Generated\Domainregistration\PlanConfiguration\PlanConfigurationNameservers $nameservers,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        foreach ($decodedJson['dnsTemplates'] as $element0) {
            $var0[] = \ShockMedia\Generated\Domainregistration\PlanConfiguration\DnsTemplateRef::fromDecodedJson($element0);
        }
        /** @var \ShockMedia\Generated\Domainregistration\PlanConfiguration\DnsTemplateRef[] $dnsTemplates */
        $dnsTemplates = $var0;

        $var0 = array();
        foreach ($decodedJson['hostingPlans'] as $element0) {
            $var0[] = \ShockMedia\Generated\Domainregistration\PlanConfiguration\HostingPlan::fromDecodedJson($element0);
        }
        /** @var \ShockMedia\Generated\Domainregistration\PlanConfiguration\HostingPlan[] $hostingPlans */
        $hostingPlans = $var0;

        $var0 = array();
        foreach ($decodedJson['specs'] as $element0) {
            $var0[] = \ShockMedia\Generated\Domainregistration\PlanConfiguration\OfferingSpecification::fromDecodedJson($element0);
        }
        /** @var \ShockMedia\Generated\Domainregistration\PlanConfiguration\OfferingSpecification[] $specs */
        $specs = $var0;

        $nameservers = \ShockMedia\Generated\Domainregistration\PlanConfiguration\PlanConfigurationNameservers::fromDecodedJson($decodedJson['nameservers']);

        return new PlanConfiguration
        (
            $servers, $dnsTemplates, $hostingPlans, $specs, $nameservers
        );
    }
}

namespace ShockMedia\Generated\Domainregistration\PlanConfiguration;


/**
 * An available DNS template.
 */
class DnsTemplateRef
{
    /**
     * @param $id The ID of the template.
     * @param $description A (user-defined) description of the template.
     */
    public function __construct
    (
...
/**
 * An available hosting plan.
 */
class HostingPlan
{
    /**
     * @param $id The ID of the plan.
     * @param $type Whether the offering is a plan or product (currently always a plan).
     * @param $name The name of the plan.
     * @param $monthly The monthly price of the plan.
     * @param $discountedMonthly The discounted monthly price of the plan, if applicable.
     * @param \ShockMedia\Generated\Domainregistration\HostingPlan\OfferingSpecificationValue[] $specValues Additional details regard the plan (features).
     */
    public function __construct
    (
        public readonly int $id,
...
/**
 * A feature of an available offering.
 */
class OfferingSpecificationValue
{
    /**
     * @param $name The name of the feature. This will correspond to an offeringSpecification entry. 
...
/**
 * A single feature description that may apply to one or more available
 * offerings.
 */
class OfferingSpecification
{
     ...
     * @param $index The relative display position of the feature.
     * @param $unit The unit of the values, if applicable.
     * @param $base2 If the values are base-2.
...
/**
 * The nameserver requirements.
 */
class PlanConfigurationNameservers
{
    /**
     * @param $min The minimum number of nameservers.
     * @param $max The maximum number of nameservers.
     */
    public function __construct
    (
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        return new PlanConfigurationNameservers
        (
            $min, $max
        );
    }
...
/**
 * Additional properties that are only used by specific registries.
 * These should only be specified if strictly necessary,
 * not just because the information might be available.
 */
...
/**
 * A validation of the domain contact.
 */
class Validation
{
    /**
     * @param $category The category of the domain contact validation.
     * @param $version A version specification.
     * @param $validatedOn The moment on which the validation completed.
     */
    public function __construct
    (
...
/**
 * A single customer specified name server.
 */
class NameServer
{
    /**
     * @param $index The priority/index - the primary name server should have index 1, the secondary 2, and so on. 
     * @param $ns The full (host) name of the name server.
     */
    public function __construct
    (
        public readonly int $index,
...
/**
 * Accepted quotes for domain registration costs.
 * Note: a selected hosting plan, if any, should not be specified here, but
 * separately by setting 'hostingPlanId'.
 */
class AcceptedQuote
{
    /**
     * @param $monthly The monthly domain registration costs that the customer accepts.
     * @param $initialPeriod The initial contract period that the customer accepts.
     * @param $renewalPeriod The renewal contract period that the customer accepts.
     */
    public function __construct
    (
        public readonly float $monthly,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        (
            $monthly, $initialPeriod, $renewalPeriod
        );
    }
}
...
enum Permission: string
{
    ...
    case ACL_AFFILIATE_CONTACT_MANAGE = 'ACL_AFFILIATE_CONTACT_MANAGE';
    case ACL_MANAGE = 'ACL_MANAGE';
    case ACL_READ = 'ACL_READ';
    case AFFILIATE_CONTACT_STATE_MANAGE = 'AFFILIATE_CONTACT_STATE_MANAGE';
    ...
    case SCHEDULED_UPDATE_READ = 'SCHEDULED_UPDATE_READ';
    case SCHEDULED_UPDATE_MANAGE = 'SCHEDULED_UPDATE_MANAGE';
    case SERVICE_MANAGE = 'SERVICE_MANAGE';
    case SLA_DASHBOARD = 'SLA_DASHBOARD';
    ...
    case TICKET_READ_TRUST_CENTER = 'TICKET_READ_TRUST_CENTER';
...
class Invoice
{
    ...
    (
        ...
        public readonly bool $payableWithOnlinePayment,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool $payableWithOnlinePayment */
        $payableWithOnlinePayment = $decodedJson['payableWithOnlinePayment'];
        ...
        return new Invoice
        (
            $invoiceNumber, $paymentDate, $vat, $customerId, $invoiceMethod, $hasPrint, $total, $date, $status, $reminder, $payableWithOnlinePayment, $paymentState
        );
    }
}
...
enum DnsErrorCode: string
{
    ...
    case INVALID_SRV_CONTENT_PORT = 'INVALID_SRV_CONTENT_PORT';
    case INVALID_SRV_CONTENT_WEIGHT = 'INVALID_SRV_CONTENT_WEIGHT';
    case INVALID_TLSA_CONTENT = 'INVALID_TLSA_CONTENT';
    ...
    case NULL_MX_RECORD_CLASH = 'NULL_MX_RECORD_CLASH';
}
...
class DomainInfo
{
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        (
            $domainName, $cancellable, $authCodeAvailable, $authCodeUpdatable, $transferLocked
        );
    }
}
...
class AcquiredOffering
{
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        (
            $acquiredId, $surcharge, $quantity, $start, $startContract, $end, $earliestCancellationDate, $offeringCategory, $offeringType, $offeringName, $offeringPrice, $effectivePrice, $serviceName, $accountName, $domainName, $managedApps, $servers, $extension
        );
    }
}
...
class ServerCountFilter
{
    public function __construct
    (
        public readonly bool|null $managed,
    ) {
    }

    public static function fromDecodedJson(array $decodedJson)
    {
        /** @var bool|null $managed */
        $managed = $decodedJson['managed'];

        return new ServerCountFilter
        (
            $managed
        );
    }
...
class BackupJobFilter
{
    public function __construct
    (
        public readonly int|null $serverId,
        public readonly string|null $serverFqdnLike,
        public readonly bool|null $hasActiveServer,
        public readonly string|null $backupMachineLike,
        public readonly bool|null $expired,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var string|null $serverFqdnLike */
        $serverFqdnLike = $decodedJson['serverFqdnLike'];

        /** @var bool|null $hasActiveServer */
        $hasActiveServer = $decodedJson['hasActiveServer'];

        /** @var string|null $backupMachineLike */
        $backupMachineLike = $decodedJson['backupMachineLike'];
...
class Server
{
     ...
     * @param \ShockMedia\Generated\Servers\ItemInfo[]|null $zabbixWebCheckItems
    ...
    (
        ...
        public readonly array|null $zabbixWebCheckItems,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        if ($decodedJson['zabbixWebCheckItems'] === NULL) {
            $var0 = NULL;
        } else {
            $var0 = array();
            foreach ($decodedJson['zabbixWebCheckItems'] as $element0) {
                $var0[] = \ShockMedia\Generated\Servers\ItemInfo::fromDecodedJson($element0);
            }
        }
        /** @var \ShockMedia\Generated\Servers\ItemInfo[]|null $zabbixWebCheckItems */
        $zabbixWebCheckItems = $var0;
        ...
        /** @var bool $enrolled */
        $enrolled = $decodedJson['enrolled'];

        return new Server
        (
            $fqdn, $type, $adminLevel, $ips, $slaRemarks, $slaName, $datacenter, $cloudName, $os, $packages, $monitored, $zabbixTriggers, $zabbixGraphItems, $zabbixWebCheckItems, $backupPlanSummary, $backupJobs, $enrolled
        );
    }
}
...
class BackupJob
{
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        (
            $fileCount, $size, $completionTime, $vendor, $level, $expired
        );
    }
}
...
class ContactFilter
{
    ...
    (
        ...
        public readonly bool|null $includeAffiliateContacts,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool|null $includeAffiliateContacts */
        $includeAffiliateContacts = $decodedJson['includeAffiliateContacts'];

        return new ContactFilter
        (
            $name, $group, $includeAffiliateContacts
        );
    }
}
...
class Contact
{
    ...
    (
        ...
        public readonly bool $placeOnAffiliateAcl,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool $placeOnAffiliateAcl */
        $placeOnAffiliateAcl = $decodedJson['placeOnAffiliateAcl'];

        return new Contact
        (
            $id, $gender, $firstName, $lastName, $companyName, $account, $email, $telephone, $notes, $groups, $placeOnAffiliateAcl
        );
    }
}
...
class ContactUpdate
{
    ...
    (
        ...
        public readonly string|null $notes,
        public readonly bool|null $placeOnAffiliateAcl,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool|null $placeOnAffiliateAcl */
        $placeOnAffiliateAcl = $decodedJson['placeOnAffiliateAcl'];

        return new ContactUpdate
        (
            $id, $person, $companyName, $groups, $emails, $telephones, $notes, $placeOnAffiliateAcl
        );
    }
}
...
class BuildDeployment
{
    public function __construct
    (
        public readonly \ShockMedia\Generated\Map\Build $build,
        public readonly \ShockMedia\Generated\Map\Upload $upload,
        public readonly \ShockMedia\Generated\Map\Deployment $deployment,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        $upload = \ShockMedia\Generated\Map\Upload::fromDecodedJson($decodedJson['upload']);

        $deployment = \ShockMedia\Generated\Map\Deployment::fromDecodedJson($decodedJson['deployment']);

        return new BuildDeployment
        (
...
class Deployment
{
    ...
    (
        ...
        public readonly \ShockMedia\Generated\Map\Template|null $template,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        if ($decodedJson['template'] === NULL) {
            $template = NULL;
        } else {
            $template = \ShockMedia\Generated\Map\Template::fromDecodedJson($decodedJson['template']);
        }
        ...
        (
            $id, $name, $description, $type, $managedApp, $build, $template, $status, $service, $domains
        );
    }
}
...
class Upload
{
    ...
    (
        ...
        public readonly string|null $bundle,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        $bundle = $decodedJson['bundle'];

        /** @var int|null $fileSize */
        $fileSize = $decodedJson['fileSize'];

        return new Upload
        (
            $id, $deploymentId, $uploadDate, $bundle, $fileSize
        );
    }
}
...
class Build
{
    ...
    (
        public readonly string $id,
        public readonly string|null $startedAt,
        public readonly \ShockMedia\Generated\Map\BuildState $buildState,
        public readonly string|null $jobUuid,
        public readonly string|null $legacyBuildTag,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        $startedAt = $decodedJson['startedAt'];

        $buildState = \ShockMedia\Generated\Map\BuildState::from($decodedJson['buildState']);

        /** @var string|null $jobUuid */
        $jobUuid = $decodedJson['jobUuid'];

        /** @var string|null $legacyBuildTag */
        $legacyBuildTag = $decodedJson['legacyBuildTag'];

        return new Build
        (
            $id, $startedAt, $buildState, $jobUuid, $legacyBuildTag
        );
    }
}
...
class UptimeStats
{
    public function __construct
    (
        public readonly float $networkUptime,
        public readonly float $serversUptime,
        public readonly int $includedServers,
        public readonly int $totalServers,
    ) {
    }

    public static function fromDecodedJson(array $decodedJson)
    {
        /** @var float $networkUptime */
        $networkUptime = $decodedJson['networkUptime'];

        /** @var float $serversUptime */
        $serversUptime = $decodedJson['serversUptime'];

        /** @var int $includedServers */
        $includedServers = $decodedJson['includedServers'];

        /** @var int $totalServers */
        $totalServers = $decodedJson['totalServers'];

        return new UptimeStats
        (
            $networkUptime, $serversUptime, $includedServers, $totalServers
        );
    }
}
...
class TicketStats
{
    ...
    (
        ...
        public readonly int|null $plannedTickets,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var int|null $plannedTickets */
        $plannedTickets = $decodedJson['plannedTickets'];
        ...
        (
            $openTickets, $plannedTickets, $closedTickets
        );
    }
}
...
{
    /**
     * @param \ShockMedia\Generated\Orders\Task[]|null $tasks
     */
    public function __construct
    (
        public readonly int $id,
        ...
        public readonly int|null $progress,
        public readonly array|null $tasks,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var \ShockMedia\Generated\Orders\Task[]|null $tasks */
        $tasks = $var0;
        ...
        return new Order
        (
            $id, $title, $start, $end, $deadline, $delivered, $hasCustomerResponsibility, $progress, $tasks, $orderType
        );
    }
}
...
{
    /**
     * @param \ShockMedia\Generated\Map\BuildState[]|null $buildState
     * @param \ShockMedia\Generated\Map\DeploymentStatus[]|null $deploymentStatus
     */
    public function __construct
    (
        public readonly string|null $nameLike,
        public readonly string|null $managedAppId,
        public readonly int|null $templateId,
        public readonly array|null $buildState,
        public readonly array|null $deploymentStatus,
        public readonly \ShockMedia\Generated\Map\DeploymentType|null $type,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var string|null $managedAppId */
        $managedAppId = $decodedJson['managedAppId'];

        /** @var int|null $templateId */
        $templateId = $decodedJson['templateId'];

        if ($decodedJson['buildState'] === NULL) {
            $var0 = NULL;
        } else {
            $var0 = array();
            foreach ($decodedJson['buildState'] as $element0) {
                $var0[] = \ShockMedia\Generated\Map\BuildState::from($element0);
            }
        }
        /** @var \ShockMedia\Generated\Map\BuildState[]|null $buildState */
        $buildState = $var0;

        if ($decodedJson['deploymentStatus'] === NULL) {
            $var0 = NULL;
        } else {
            $var0 = array();
            foreach ($decodedJson['deploymentStatus'] as $element0) {
                $var0[] = \ShockMedia\Generated\Map\DeploymentStatus::from($element0);
            }
        }
        /** @var \ShockMedia\Generated\Map\DeploymentStatus[]|null $deploymentStatus */
        $deploymentStatus = $var0;
        ...
        (
            $nameLike, $managedAppId, $templateId, $buildState, $deploymentStatus, $type
        );
    }
}
...
class RequestAuth
{
    ...
    (
        ...
        public readonly string|null $myAccountUsername,
        public readonly int|null $customerId,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var string|null $myAccountUsername */
        $myAccountUsername = $decodedJson['myAccountUsername'];
        ...
        (
            $ok, $error, $type, $unknownType, $accountId, $myAccountId, $myAccountUsername, $customerId, $shadowCustomer
        );
    }
}
...
class Invocation
{
    public function __construct
    (
        public readonly string|null $resourceClass,
        public readonly string|null $method,
        public readonly string|null $params,
    ) {
    }
...
class UpdateAffiliateInput
{
    public function __construct
    (
        public readonly int $id,
        public readonly bool|null $mayContact,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var bool|null $mayContact */
        $mayContact = $decodedJson['mayContact'];

        return new UpdateAffiliateInput
        (
            $id, $mayContact
        );
    }
...
class Activity
{
    ...
    (
        ...
        public readonly string|null $serverName,
        public readonly string|null $managedAppId,
        public readonly string|null $managedAppName,
        public readonly string|null $domainName,
    ) {
    }
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        ...
        /** @var string|null $managedAppName */
        $managedAppName = $decodedJson['managedAppName'];
        ...
        return new Activity
        (
            $id, $subject, $description, $start, $duration, $owner, $system, $ticket, $rebootPerformed, $serverId, $serverName, $managedAppId, $managedAppName, $domainName
        );
    }
}
...
class ActivityFilter
{
    public function __construct
    (
        public readonly \ShockMedia\Generated\Activities\ServiceType|null $serviceType,
        public readonly string|null $serviceNameLike,
        public readonly string|null $fqdn,
    ...
    public static function fromDecodedJson(array $decodedJson)
    {
        if ($decodedJson['serviceType'] === NULL) {
            $serviceType = NULL;
        } else {
            $serviceType = \ShockMedia\Generated\Activities\ServiceType::from($decodedJson['serviceType']);
        }

        /** @var string|null $serviceNameLike */
        $serviceNameLike = $decodedJson['serviceNameLike'];
        ...
        (
            $serviceType, $serviceNameLike, $fqdn, $query
        );
    }
}
